...
function ItemRow({ primary, secondary, badge, onEdit, onDelete, isDeleting }: RowProps) {
...
export default function SettingsPage() {
  ...
  const [walletSheet, setWalletSheet] = useState<WalletSheetState>({ open: false });
  const [categorySheet, setCategorySheet] = useState<CategorySheetState>({ open: false });
  ...
  const deleteWallet = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: WALLET_KEYS.all }),
  });
              ...
              wallets.map((wallet, idx) => (
                <div key={wallet.id}>
                  ...
                  <ItemRow
                    ...
                    onEdit={() => setWalletSheet({ open: true, wallet })}
                    onDelete={() => handleDeleteWallet(wallet)}
                    isDeleting={deleteWallet.isPending && deleteWallet.variables === wallet.id}
                  />
                </div>
              ))
            )}
          ...
          <div style={sectionCardStyle}>
            ...
            {categoriesLoading ? (
              ...
              <>
                  ...
                  incomeCategories.map((cat, idx) => (
                    <div key={cat.id}>
                      ...
                      <ItemRow
                        ...
                        onEdit={() => setCategorySheet({ open: true, category: cat })}
                ...
                <button
                  onClick={() => setCategorySheet({ open: true, defaultType: "income" })}
                  style={addRowStyle}
                >
                  <PlusIcon />
                  Add income category
                ...
                {/* Expense sub-section */}
                <div style={{ ...subHeaderStyle, borderTop: "0.5px solid var(--color-border)" }}>
                  Expense
                </div>
                ...
                <button
                  onClick={() => setCategorySheet({ open: true, defaultType: "expense" })}
                  style={addRowStyle}
                >
                  <PlusIcon />
